...
The question privides a website which has file upload feature and the attached file have pdf files which have command strings.
...
This is a php script for "sign your payload" feature(sign.php). This script generates signature strings when a PDF file which has only "ECHO" command uploads to this page. 
...
When "echo-ciao.pdf" is uploaded, response data prints signature strings whici is from given sample file  "echo-ciao.sig"
...
This is another php script for execute for "your signed payload" feature(execute.php). This script allows only "EXECUTE" command of PDF with signature strings of the PDF.
...
A SHA1 hash collision script from https://github.com/nneonneo/sha1collider uses to solve this problem.
